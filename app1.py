...
st.info("""
**Instrukcja:**
1. Wgraj plik Excel z dostępnością.
2. Ustaw limity i kliknij **Generuj grafik**.
3. Gdy pojawi się podgląd, możesz pobrać plik Excel.
""")
...
if uploaded_file is not None:
    ...
    pracownicy = df_input.columns[2:].tolist()
    ...
    if generate_btn:
        ...
        df_input['PRZYPISANI_PRACOWNICY'] = wynikowa_kolumna
        
        # ZAPISUJEMY WYNIKI DO SCHOWKA (Session State)
        st.session_state['gotowy_df'] = df_input
        st.session_state['statystyki'] = liczniki

    # WYŚWIETLANIE WYNIKÓW (jeśli istnieją w schowku)
    if 'gotowy_df' in st.session_state:
        df_final = st.session_state['gotowy_df']
        stats = st.session_state['statystyki']

        st.divider()
        st.subheader("Podgląd wygenerowanego grafiku")
        st.caption("(!) Oznacza osobę dobraną awaryjnie (krótki odstęp między zmianami).")
        st.dataframe(df_final[['DATA', 'ZMIANA', 'PRZYPISANI_PRACOWNICY']], use_container_width=True)
        
        # Przygotowanie pliku do pobrania
        output = io.BytesIO()
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df_final.to_excel(writer, index=False, sheet_name='Grafik_Wynik')
